        ...
        <div class="flex justify-between mt-1">
            ...
            <div class="p-2 relative">
                <p class="text-base font-semibold"><span class="text-xs font-light">ISS</span>
                    {{ $license->created_at->format(get_setting('general.dateFormat'))}}</p>
                <p class="text-base font-semibold"><span class="text-xs font-light">EXP</span>
                    {{ $license->expires_at->format(get_setting('general.dateFormat'))}}</p>
                <div class="absolute bottom-0">
                    <p class="uppercase text-lg font-bold">
                        @if ($license->status == \App\Enum\LicenseStatus::Valid)
                            @if ($license->expires_at <= now())
                                <span class="text-red-500">EXPIRED</span>
                            @else
                                <span class="text-green-500">VALID</span>
                            @endif
                        @elseif($license->status == \App\Enum\LicenseStatus::Expired)
                            <span class="text-red-500">EXPIRED</span>
                        @elseif($license->status == \App\Enum\LicenseStatus::Suspended)
                            <span class="text-red-500">Suspended</span>
                        @elseif($license->status == \App\Enum\LicenseStatus::Revoked)
                            <span class="text-red-500">Revoked</span>
                        @elseif($license->status == \App\Enum\LicenseStatus::Pending)
