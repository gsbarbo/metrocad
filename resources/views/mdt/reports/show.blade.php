...
@section('content')
    ...
    <div class="p-2 grid grid-cols-5 gap-2">
        <div class="col-span-4 space-y-4">
            ...
            <livewire:mdt.reports.property-model :reportId="$report->id" :report="$report"/>

            <livewire:mdt.reports.person-model :reportId="$report->id" :report="$report"/>

            <livewire:mdt.reports.vehicle-model :reportId="$report->id" :report="$report"/>

            <div class="bg-slate-700 rounded-lg text-white">
                <form action="{{route('mdt.reports.update', $report->id)}}" method="post" id="narrativeForm">
                    @csrf
                    @method('PUT')
                    <div class="bg-blue-700 text-white border-b-4 border-blue-800 rounded-t-lg py-2">
