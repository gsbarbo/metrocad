...
@section('main')
    <header class="w-full my-3">
        <h1 class="text-2xl font-bold text-white">Discord Roles</h1>
    ...
    <div class="">
        <form action="{{ route('admin.settings.update') }}" id="mdeditor" method="POST">
            ...
            <div class="p-3 flex justify-between">
                <div>
                    <label class="text-lg font-semibold" for="feature_use_discord_roles">
                        Use Discord Roles
                    </label>
                    <p>Manage CAD Roles with Discord Roles. It is best not to switch this a lot. Pick one
                        and stick with it. Bugs may appear if you switch between using Discord roles and back to CAD roles.
                    </p>
                </div>
                <div class="ml-3">
                    <select class="!w-28 select-input" id="feature_use_discord_roles" name="feature_use_discord_roles">
            ...
            <form action="{{ route('admin.settings.update') }}" class="divide-y" method="POST">
                ...
                <div class="p-3">
                    <div class="flex justify-between">
                        <div>
                            <p class="text-lg font-semibold">Discord Member Role</p>
                            <p>This will allow members immediate access to the CAD if they have this role in your Discord
                    ...
                    <select class="select-input" id="discord_auto_role_id" name="discord_auto_role_id">
                ...
                <div class="p-3">
                    <div class="flex justify-between">
                        <div>
                            <p class="text-lg font-semibold">Discord Suspend Role</p>
                            <p>This will suspend members from the CAD if they have this role in your Discord server. If you
                                do not want to use this setting lave as 'None'.</p>
                        </div>
                    </div>

                    <select class="select-input" id="discord_suspended_role_id" name="discord_suspended_role_id">
                        <option value="">-- Choose One --</option>
                        <option selected value="0">None</option>
                        @foreach ($discord_roles as $id => $discord_role)
                            @if ($id != 0 && $discord_role->managed != true)
                ...
                <div class="p-3 flex justify-between">
                    <div>
                        <label class="text-lg font-semibold" for="feature_use_discord_department_roles">
                            Use Discord Department Roles
                        </label>
                        <p>Allows members to set up thier own Officer based on the roles in Discord. They
                    ...
                    <div class="ml-3">
                        <select class="!w-28 select-input" id="feature_use_discord_department_roles"
