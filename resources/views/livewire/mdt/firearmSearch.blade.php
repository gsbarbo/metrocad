...
new class extends Component {
    ...
    public bool $searchStolenFirearms = true;
    public bool $searchImpoundedFirearms = true;
    ...
    public function search(): void
    {
        ...
        $query->where(function (Builder $q) {
            if ($this->serial_number) {
                $q->orWhere(function (Builder $q) {
                    $q->where('serial_number', 'like', '%'.$this->serial_number.'%');
                });
            }
        });

        $query->where(function (Builder $q) {
            if ($this->searchStolenFirearms) {
                $q->orWhere('status', FirearmStatus::Stolen->value);
            }
            if ($this->searchImpoundedFirearms) {
                $q->orWhere('status', FirearmStatus::Impounded->value);
            }

            $q->orWhere('status', FirearmStatus::Valid->value);
        ...
        $this->searchText = "Your search returned: {$results->count()} firearms.";
    }
}
?>
...
<div>
    <h2 class="text-2xl flex items-center">
        ...
        <span class="ml-3">Firearm Search</span>
    </h2>
    <div class="grid grid-cols-12 gap-4 mt-4">
        <div class="p-2 col-span-10">
            <div class="grid grid-cols-6 gap-4 border-2 p-2">
                ...
                <div class="col-span-2">
                    <label class="label-dark font-bold text-lg">Serial Number:</label>
                    <input class="mdt-text-input text-3xl" placeholder="Serial Number" autofocus type="text"
        ...
        <div class="p-2 col-span-2">
            <div>
                <input class="" type="checkbox" id="searchImpoundedFirearms" wire:model="searchImpoundedFirearms">
                <label class="label-dark inline-block underline cursor-pointer" for="searchImpoundedFirearms">Local
                    Impound</label>
            </div>
            <div>
                <input class="" type="checkbox" id="searchStolenFirearms" wire:model="searchStolenFirearms">
                <label class="label-dark inline-block underline cursor-pointer" for="searchStolenFirearms">Stolen
                    Firearms</label>
            </div>
            <div class="">
    ...
    <div class="grid grid-cols-2 gap-2 mt-3">
        ...
        @foreach ($results as $firearm)
            <div class="bg-[#222423] rounded-lg p-4 text-white hover:bg-[#0C1011] transition-colors">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold">{{ $firearm->serial_number }}</h2>
                    <div>
                        @if($firearm->status == FirearmStatus::Valid)
                            <span
                                class="text-xs font-medium px-2.5 py-0.5 rounded-sm bg-green-900 text-green-300">Valid</span>
                        @endif
                        @if($firearm->status == FirearmStatus::Stolen)
                            <span
                                class="text-xs font-medium px-2.5 py-0.5 rounded-sm bg-red-900 text-red-300">Stolen</span>
                        @endif
                        @if($firearm->status == FirearmStatus::Impounded)
                            <span
                                class="text-xs font-medium px-2.5 py-0.5 rounded-sm bg-red-900 text-red-300">Impounded</span>
                        @endif
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-0.5 mt-3">
                    <a href="{{route('mdt.civilianReturn', $firearm->civilian)}}">
                        <p class="text-sm">RO: {{ $firearm->civilian->last_name }}
                            , {{ $firearm->civilian->first_name }}</p>
                    </a>
                </div>
            </div>
        @endforeach
