@extends('layouts.public')

@section('main')
    <div class="max-w-2xl mx-auto ">
        <div class="border-navbar border-2 rounded-lg p-4">
            <div>
                <img alt="Community Logo" class="mx-auto w-1/2" src="{{ get_setting('community_logo') }}">
            </div>
            @guest
                <div class="space-y-3 text-center">
                    <h2 class="text-2xl font-semibold">{{ __('Welcome to') }} {{ get_setting('community_name') }}!</h2>

                    <p class="text-sm">{{ get_setting('community_intro') }}</p>

                    <a class="inline-block" href="{{ route('auth.discord') }}">
                        <button
                            class="mx-auto flex items-center bg-discord rounded-lg px-6 py-2 text-sm font-medium text-white hover:opacity-85">

                            <svg class="h-6 w-6 mr-2" height="800px" preserveAspectRatio="xMidYMid" version="1.1"
                                 viewBox="0 -28.5 256 256" width="800px"
                                 xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M216.856339,16.5966031 C200.285002,8.84328665 182.566144,3.2084988 164.041564,0 C161.766523,4.11318106 159.108624,9.64549908 157.276099,14.0464379 C137.583995,11.0849896 118.072967,11.0849896 98.7430163,14.0464379 C96.9108417,9.64549908 94.1925838,4.11318106 91.8971895,0 C73.3526068,3.2084988 55.6133949,8.86399117 39.0420583,16.6376612 C5.61752293,67.146514 -3.4433191,116.400813 1.08711069,164.955721 C23.2560196,181.510915 44.7403634,191.567697 65.8621325,198.148576 C71.0772151,190.971126 75.7283628,183.341335 79.7352139,175.300261 C72.104019,172.400575 64.7949724,168.822202 57.8887866,164.667963 C59.7209612,163.310589 61.5131304,161.891452 63.2445898,160.431257 C105.36741,180.133187 151.134928,180.133187 192.754523,160.431257 C194.506336,161.891452 196.298154,163.310589 198.110326,164.667963 C191.183787,168.842556 183.854737,172.420929 176.223542,175.320965 C180.230393,183.341335 184.861538,190.991831 190.096624,198.16893 C211.238746,191.588051 232.743023,181.531619 254.911949,164.955721 C260.227747,108.668201 245.831087,59.8662432 216.856339,16.5966031 Z M85.4738752,135.09489 C72.8290281,135.09489 62.4592217,123.290155 62.4592217,108.914901 C62.4592217,94.5396472 72.607595,82.7145587 85.4738752,82.7145587 C98.3405064,82.7145587 108.709962,94.5189427 108.488529,108.914901 C108.508531,123.290155 98.3405064,135.09489 85.4738752,135.09489 Z M170.525237,135.09489 C157.88039,135.09489 147.510584,123.290155 147.510584,108.914901 C147.510584,94.5396472 157.658606,82.7145587 170.525237,82.7145587 C183.391518,82.7145587 193.761324,94.5189427 193.539891,108.914901 C193.539891,123.290155 183.391518,135.09489 170.525237,135.09489 Z"
                                        fill-rule="nonzero" fill="#FFFFFF">

                                    </path>
                                </g>
                            </svg>
                            <span>{{ __('Continue with Discord') }}</span>
                        </button>
                    </a>
                </div>
            @endguest

            @auth
                <div class="space-y-3 text-center">
                    <h2 class="text-2xl font-semibold">Welcome back, {{ auth()->user()->name }}!</h2>

                    @switch(auth()->user()->status)
                        @case(\App\Enum\User\UserStatuses::PENDING)
                            <p>Your account is pending. Please contact your supervisor to gain access to the
                                CAD/MDT.</p>
                            <a class="inline-block" href="#">
                                <button
                                    class="mx-auto flex items-center bg-red-600 rounded-lg px-6 py-2 text-sm font-medium text-white hover:opacity-85">

                                    <svg class="h-6 w-6 mr-2" fill="none" stroke-width="1.5" stroke="currentColor"
                                         viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M8.25 9V5.25A2.25 2.25 0 0 1 10.5 3h6a2.25 2.25 0 0 1 2.25 2.25v13.5A2.25 2.25 0 0 1 16.5 21h-6a2.25 2.25 0 0 1-2.25-2.25V15m-3 0-3-3m0 0 3-3m-3 3H15"
                                            stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>

                                    <span>Logout</span>
                                </button>
                            </a>
                            @break

                        @case(\App\Enum\User\UserStatuses::MEMBER)
                            <p>{{ get_setting('community_intro') }}</p>
                            <a class="inline-block" href="{{ route('portal.dashboard') }}">
                                <button
                                    class="mx-auto flex items-center bg-primary rounded-lg px-6 py-2 text-sm font-medium text-white hover:opacity-85">

                                    <svg class="h-6 w-6 mr-2" fill="none" stroke-width="1.5" stroke="currentColor"
                                         viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25"
                                            stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                    <span>CAD</span>
                                </button>
                            </a>

                            <a class="inline-block" href="#">
                                <button
                                    class="mx-auto flex items-center bg-red-600 rounded-lg px-6 py-2 text-sm font-medium text-white hover:opacity-85">

                                    <svg class="h-6 w-6 mr-2" fill="none" stroke-width="1.5" stroke="currentColor"
                                         viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M8.25 9V5.25A2.25 2.25 0 0 1 10.5 3h6a2.25 2.25 0 0 1 2.25 2.25v13.5A2.25 2.25 0 0 1 16.5 21h-6a2.25 2.25 0 0 1-2.25-2.25V15m-3 0-3-3m0 0 3-3m-3 3H15"
                                            stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>

                                    <span>Logout</span>
                                </button>
                            </a>
                            @break

                        @default
                            <p>Your account is {{ auth()->user()->status->name() }}. Please contact your supervisor to
                                gain
                                access to the
                                CAD/MDT.
                            </p>
                    @endswitch

                    @if (get_setting('discord_auto_role_id', 0) != 0 && get_setting('feature_use_discord_roles'))
                        <p class="form-help-text-dark">Discord auto role is on.</p>
                    @endif

                </div>
            @endauth
        </div>
        <div class="mt-3 flex justify-between mx-3">
            <a class="font-light text-sm text-gray-500 underline"
               href="https://github.com/gsbarbo/metrocad/releases/tag/v{{ config('metrocad.version') }}"
               target="_blank">v{{ config('metrocad.version') }}</a>
            <a class="font-light text-sm text-gray-500 underline" href="#">Powered by MetroCAD</a>
        </div>
    </div>
@endsection
