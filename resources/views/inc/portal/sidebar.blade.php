<aside
    ...
    :class="sidebarExpanded ? 'w-72' : 'w-16'" @mouseenter="sidebarExpanded = true" @mouseleave="sidebarExpanded = false">
    {{-- Logo --}}
    ...
    <nav class="flex-1 py-3 px-2 space-y-0.5 overflow-hidden">
        @foreach (\App\Helpers\Menu\PortalMenuHelper::links() as $link)
    ...
    <div class="py-3 px-2 border-t border-border overflow-hidden">
        @foreach (\App\Helpers\Menu\PortalMenuHelper::bottomLinks() as $link)
            <x-sidebar-link :route="$link['route']" :icon="$link['icon']">
                {{ $link['label'] }}
            </x-sidebar-link>
        @endforeach
