...
class ActiveUnitAutoOffDutyMiddleware
{
    ...
    {
        $active_units = ActiveUnit::without(['officer', 'user_department', 'calls', 'user'])->get(['updated_at']);

        foreach ($active_units as $active_unit) {
            if ($active_unit->updated_at < Carbon::now()->subMinute(get_setting('mdt.activeUnitTimeout'))) {
