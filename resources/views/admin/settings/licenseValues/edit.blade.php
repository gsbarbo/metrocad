...
@section('main')
    <x-breadcrumb pageTitle="Edit License" route="{{ route('admin.dashboard') }}">
        ...
        <x-breadcrumb-text>Edit {{ $licenseValue->name }}</x-breadcrumb-text>
    </x-breadcrumb>
    <div>
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.settings.licenseValues.update', $licenseValue->id) }}" class="space-y-3"
            method="POST">
            @csrf
            @method('put')
            ...
            <div>
                ...
                <input class="form-text-input-dark" name="name" required type="text"
                    value="{{ old('name', $licenseValue->name) }}">
                @error('name')
            ...
            <div>
                ...
                <input class="form-text-input-dark" name="format" required type="text"
                    value="{{ old('format', $licenseValue->format) }}">
                @error('format')
                    <p class="text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input class="form-text-input-dark" name="prefix" type="text"
                    value="{{ old('prefix', $licenseValue->prefix) }}">
                @error('prefix')
                    <p class="text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="border-red-600 border-2 p-3">
            ...
            <form action="{{ route('admin.settings.licenseValues.destroy', $licenseValue->id) }}"
                ...
                <div>
                    <label class="label-dark" for="confirm">Please type the license name
                        ({{ $licenseValue->name }}) to confirm</label>
