    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="space-y-2">
            ...
            <div class="divide-y">
                ...
                <p class="">DOB: {{ $civilian->date_of_birth->format(get_setting('general.dateFormat')) }}</p>
            ...
            <div class="border-red-600 border-2 p-3">
                ...
                <form action="{{ route('civilians.destroy', $civilian->id) }}" class="mt-5 block space-y-3"
                      method="POST">
                    @csrf
                    @method('delete')
                        ...
                        <div class="col-span-2" x-data="{ openSub: false }">
                            ...
                            <div class="border border-green-400" x-show="openSub">
                                <form action=" {{ route('civilians.license.store', $civilian->id) }}"
                                    ...
                                    @csrf
                                    <x-forms.select name="license_type_id" label="Type">
                                    ...
                                    <div class="">
                                        <p class="form-help-text-dark">Status can not be changed without an officer
                                            after
                                            this.
                                        </p>
                                        <button class="btn btn-green btn-rounded btn-md" type="submit">Get License
                                        </button>
                                    </div>
                                </form>
                            </div>
            ...
            <div class="" x-data="{ open: false }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2" x-show="open">
                    <div class="col-span-2" x-data="{ openSub: false }">
                        ...
                        <div class="border border-green-400" x-show="openSub">
                            <form action=" {{ route('civilians.vehicle.store', $civilian->id) }}"
                                  class="grid gap-3 p-3 grid-cols-1 md:grid-cols-3" method="POST">
                                @csrf
                                <x-forms.select name="vehicle_type_id" label="Vehicle" required>
                                ...
                                <x-forms.select name="status" label="Status" required>
                                    <option value="">Status</option>
                                ...
                                <x-forms.input name="color" label="Color" required></x-forms.input>
                                <x-forms.input name="plate" label="Plate" required></x-forms.input>

                                <div class="">
                                    <p class="form-help-text-dark">Status can not be changed without an officer after
                                        this.
                                    </p>
                                    <button class="btn btn-green btn-rounded btn-md" type="submit">Register Vehicle
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    @forelse ($civilian->vehicles as $vehicle)
                        <x-civilian.vehicle-card :vehicle="$vehicle" :civilian="$civilian"></x-civilian.vehicle-card>
                    @empty
            ...
            <div class="" x-data="{ open: false }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2" x-show="open">
                    <div class="col-span-2" x-data="{ openSub: false }">
                        ...
                        <div class="border border-green-400" x-show="openSub">
                            <form action=" {{ route('civilians.firearm.store', $civilian->id) }}"
                                  class="grid gap-3 p-3 grid-cols-1 md:grid-cols-3" method="POST">
                                @csrf
                                <x-forms.input name="model" label="Model" required></x-forms.input>
                                <x-forms.select name="status" label="Initial Status" required>
                                    <option value="">Status</option>
                                    @foreach (App\Enum\FirearmStatus::cases() as $status)
                                        <option value="{{ $status->value }}">{{ $status->name() }}</option>
                                    @endforeach
                                </x-forms.select>
                                <x-forms.input name="serial_number" label="Serial Number"
                                               placeholder="Leave empty for default"></x-forms.input>
                                <div class="">
                                    <p class="form-help-text-dark">Status can not be changed without an officer after
                                        this.
                                    </p>
                                    <button class="btn btn-green btn-rounded btn-md" type="submit">Register Firearm
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    @forelse ($civilian->firearms as $firearm)
                        <x-civilian.firearm-card :firearm="$firearm" :civilian="$civilian"></x-civilian.firearm-card>
                    @empty
                    ...
                    <div class="col-span-3" x-data="{ openSub: false }">
                        ...
                        <div class="border border-green-400" x-show="openSub">
                            <form action=" {{ route('civilians.medicalRecords.store', $civilian->id) }}"
                                ...
                                @csrf
                                <x-forms.select name="name" label="Type" required>
                                    ...
                                    <option
                                        {{ old('name') == 'Medications' ? 'selected' : '' }} value="Medications">
                                        Medications
                                    </option>
                                ...
                                <x-forms.input name="value" label="Value" required></x-forms.input>
                                <div class="">
                                    <button class="btn btn-green btn-md btn-rounded" type="submit">Register Record
                                    </button>
                                </div>
                            </form>
                        </div>
